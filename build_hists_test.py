from build_hists import build_histograms
...
def diagnose_histogram(bin_centers, hist, lower_bound=None, upper_bound=None):
    """
    Visualize your histogram to understand the issue.
    """
...
path_to_file = "Cs-137.txt"

# Build histograms
print("Building histograms...")
histograms, bin_centers = build_histograms(path_to_file, verbose=True)

# Pick a channel to inspect
print("\n" + "="*60)
print("CHANNEL SELECTION")
print("="*60)
key = next(iter(histograms.keys()))
hist = histograms[key]
...
print("="*60)
diagnose_histogram(bin_centers, hist, lower_bound=2475, upper_bound=2900)

# Ask user if they want to proceed or adjust
print(f"\nProposed fit range: [2475, 2900]")
proceed = input("Proceed with this range? (y/n) or enter new range as 'min,max': ").strip().lower()

if proceed == 'y':
    lower_bound, upper_bound = 2475, 2900
elif ',' in proceed:
    try:
...
if lower_bound is None:
    # Auto-detect
    result = fit_one_channel2(bin_centers, hist, auto_region=True)
else:
    # Manual bounds
    result = fit_one_channel2(bin_centers, hist, auto_region=False, 
