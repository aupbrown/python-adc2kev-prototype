...
from build_hists import build_cathode_histograms
...
def main():
    ...
    # creating dictionary that stores callibration parameters in the form:
    ...
    anode_hists1, anode_bin_centers1, cathode_anode_pairs1 = build_anode_histograms(args.file1, verbose=True)

    if args.verbose:
        print(f"Building anode histograms for file 2: {args.file2}")
    anode_hists2, anode_bin_centers2, cathode_anode_pairs2 = build_anode_histograms(args.file2, verbose=True)
    ...
    if args.verbose:
        ...
        print(f"Common channels: {len(common_anode_channels)}")
        print(f"Dropped: {len(set(anode_hists1.keys()) ^ set(anode_hists2.keys()))} uncommon channels") # ^ is the symmetric difference operator
    
    # loop over all anode channel histograms, fit each histogram, compute ADC photopeak, and compute callibration parameters.
    t0 = time.perf_counter()
    ...
    for channel in common_anode_channels:
        ...
        if popt1 is None:
            print(f"  → Retrying with manual bounds [{lower_bound1}, {upper_bound1}]")
            popt1, pcov1, bins_fit1, hist_fit1 = fit_one_channel2(
        ...
        # If auto failed, try manual bounds as fallback
        if popt2 is None:
            print(f"  → Retrying with manual bounds [{lower_bound2}, {upper_bound2}]")
            popt2, pcov2, bins_fit2, hist_fit2 = fit_one_channel2(
        ...
        #  use ADC values to get callibration values for current channel
        if(not(fit_fail)):
    ...
    if args.verbose:
        ...
        print("Anode calibration finished")
        
    
    # ----- Step 3: create cathode histograms ----
    t0 = time.perf_counter()
    
    cathode_hists1, cathode_bin_centers1 = build_cathode_histograms(cathode_anode_pairs1, calibrations, args.kev1)
    cathode_hists2, cathode_bin_centers2 = build_cathode_histograms(cathode_anode_pairs2, calibrations, args.kev2)
    
    t1 = time.perf_counter()
    cathode_hist_build_time = t1 - t0
    
    if args.verbose:
        print(f"File1 cathode channels: {len(cathode_hists1)}, File2 cathode channels: {len(cathode_hists2)}")
    ...
    print(f"failed cathode fits: {failed_cathode_fits}")
    
    # ----- Step 5: save calibration parameters to output file ----
    if args.verbose:
        print(f"Writing calibration values to: '{args.out}'")
