import numpy as np
import argparse
from build_hists import build_anode_histograms, build_cathode_histograms
...
from cathode_fitting_test import diagnose_cathode_histograms, test_cathode_fit_logic
import time
import json
...
def main():
    ...
    print("\n" + "="*70)
    print("DIAGNOSING CATHODE HISTOGRAMS")
    print("="*70)

    diagnose_cathode_histograms(cathode_hists1, cathode_bin_centers1, sample_size=10)

    # Also test one specific histogram in detail
    if len(cathode_hists1) > 0:
        test_channel = list(cathode_hists1.keys())[0]
        test_hist = cathode_hists1[test_channel]
        print(f"\nDetailed analysis of channel {test_channel}:")
        test_cathode_fit_logic(test_hist, cathode_bin_centers1, show_plot=True)
